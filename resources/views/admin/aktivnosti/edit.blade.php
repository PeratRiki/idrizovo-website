...
<div class="p-6" style="background:#f0f4fa; min-height:100vh;">
    ...
    <div style="display:flex; align-items:center; gap:12px; margin-bottom:24px;">
        ...
        <h1 style="font-size:1.75rem; font-weight:800; color:#1a2e4a;">✏️ Уреди активност</h1>
    </div>
    ...
    <form method="POST" action="{{ route('admin.aktivnosti.update', $aktivnost->id) }}" enctype="multipart/form-data"
        ...
        <div>
            ...
            <input type="text" name="title" value="{{ old('title', $aktivnost->title) }}" required
        ...
        <div>
            ...
            <textarea name="description" required rows="6"
                style="width:100%; border:1.5px solid #d1dff0; border-radius:10px; padding:10px 14px; font-size:0.9rem; color:#1a2e4a; outline:none; resize:vertical;">{{ old('description', $aktivnost->description) }}</textarea>
        </div>
        ...
        <div>
            ...
            @if($aktivnost->image)
                <img src="{{ asset('storage/' . $aktivnost->image) }}"
        ...
        <div>
            ...
            <input type="number" name="sort_order" value="{{ old('sort_order', $aktivnost->sort_order) }}"
                style="width:100%; border:1.5px solid #d1dff0; border-radius:10px; padding:10px 14px; font-size:0.9rem; color:#1a2e4a; outline:none;">
        </div>

        <div style="display:flex; align-items:center; gap:10px;">
            <input type="checkbox" name="is_featured" value="1" id="is_featured" {{ old('is_featured', $aktivnost->is_featured) ? 'checked' : '' }}
        ...
        <div style="display:flex; align-items:center; gap:10px;">
            <input type="checkbox" name="is_active" value="1" id="is_active" {{ old('is_active', $aktivnost->is_active) ? 'checked' : '' }}
                style="width:18px; height:18px; cursor:pointer;">
