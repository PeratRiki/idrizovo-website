...
<div id="mainForm" class="max-w-2xl mx-auto fade-in">
  ...
  <div id="step1" class="card p-6 mb-4">
    ...
    <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
      <div>
        ...
        <input id="ime" class="form-input" placeholder="Вашето ime" maxlength="60"/>
        <div class="error-msg" id="err-ime">Полето е задолжително.</div>
      </div>
      <div>
        <label class="form-label">Презиме <span class="req">*</span></label>
        <input id="prezime" class="form-input" placeholder="Вашето prezime" maxlength="60"/>
        <div class="error-msg" id="err-prezime">Полето е задолжително.</div>
      </div>
      <div>
        <label class="form-label">Email адреса</label>
        <input id="email" type="email" class="form-input" placeholder="primer@mail.com (незадолжително)"/>
  ...
  <div id="step3" class="card p-6 mb-4">
    ...
    <div class="mt-5">
      <label class="form-label">Kako сакате да добиете потврда? <span class="req">*</span></label>
      ...
      <div class="error-msg show" id="err-notify" style="display:none">Одберете начин за потврда.</div>
    </div>
  </div>
...
<div id="successScreen" class="max-w-xl mx-auto fade-in" style="display:none">
  <div class="card p-8 text-center">
    ...
    <!-- Code box (shown always for reference, labelled per channel) -->
    ...
    <!-- Cancel visit button -->
    <button class="btn-danger w-full mt-2" onclick="openCancelOverlay()">
...
<div id="cancelOverlay">
  <div class="card p-7 max-w-md w-full mx-3 fade-in">
    ...
    <div class="flex gap-3 mt-5">
      <button class="btn-danger flex-1" onclick="confirmCancel()">Да, откажи ја</button>
      <button class="btn-primary flex-1" onclick="closeCancelOverlay()">Назад</button>
    </div>
  </div>
</div>
...
function submitForm(){
  ...
  if(!notify){
    notifyEl.style.display='block'; ok=false;
  } else { notifyEl.style.display='none'; }
 
  // If SMS chosen, need phone number
  if(notify && notify.value==='sms' && !mobilen){
    alert('За SMS потврда треба да внесете мобилен број.');
    ok=false;
  }
  // If email chosen, need email
  if(notify && notify.value==='email' && !email){
    alert('За email потврда треба да внесете email адреса.');
    ok=false;
  }
  ...
  document.getElementById('summaryBox').innerHTML = `
    <div class="flex justify-between"><span class="text-gray-500">Поsetitel:</span><strong>${ime} ${prezime}</strong></div>
